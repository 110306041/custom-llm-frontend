 ...
 * @returns {object} - Recommended full allocation {RR1: value, RR2: value, ...}
 */
export const getFixedRecommendations = (riskScore) => {
  let recommendations = {};
...
export const generateRecommendationText = (riskScore, recommendations) => {
  ...
  if (Object.keys(recommendations).length === 0) {
    ...
    for (const [fund, amount] of Object.entries(recommendations)) {
      description += `🔹 ${fund}: NT$${amount.toLocaleString()}\n`;
    }
    description += "\nThis is your target allocation based on your risk profile.\n\n";
  }
